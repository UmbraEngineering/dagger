...
// Add the session model to the model registry
app.models._addModel(__dirname, 'auth-tokens.js');

// Load the session model
var AuthToken = app.models.require(app.conf.auth.authTokenEndpoint)();
...
Request.prototype.startRequest = function() {
	...
	var method = this.method === 'delete' ? 'del' : this.method;
	if (! this.resource[method]) {
		return this.sendError(405);
	}

	if (! app.conf.auth.enabled) {
		this.resource[method](this); return;
	}
...
Request.prototype.authenticate = function() {
	...
	process.nextTick(function() {
		// Find the auth token
		var token;
		if (conf.allowAuthTokenHeader) {
			token = req.headers[conf.authTokenHeader];
		}
		if (conf.allowAuthTokenParams) {
			token = req.query[conf.authTokenParam];
		}
		...
		AuthToken.findOne({ token: token }).populate('user').exec(function(err, token) {
			if (err) {
			...
			// Store the token and user data on the request object for further use
			self.auth = token;

			// Update the TTL on the auth token
			token.updateTTL().then(
				function() {
					// ...
				},
				function(err) {
