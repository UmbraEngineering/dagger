...
// Valid HTTP method hooks
var methods = require('./http-meta').methods;
...
exports.create = function(name, props) {
	...
	if (props.hooks) {
		Object.keys(props.hooks).forEach(function(hook) {
			...
			if (hook[0] !== 'before' && hook[0] !== 'after') {
				throw new Error('Schema hooks must be defined as before::<hook> or after::<hook>');
			}
			...
			// Store HTTP hooks for the resource
			if (methods.indexOf(hook[1]) >= 0) {
				httpHooks[hook[1]] = { };
				httpHooks[hook[1]][hook[0]] = funcs;
				return;
			}

			if (hook[0] === 'before') {hook[0] = 'pre';}
			if (hook[0] === 'after') {hook[0] = 'post';}
