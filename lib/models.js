...
// -------------------------------------------------------------
	...
	Object.keys(props).forEach(function(i) {
		if (i !== 'schema' && i !== 'useTimestamps' && i !== 'methods' && i !== 'expose' && i !== 'hooks' && i !== 'ensureIndex') {
			model[i] = props[i];
		}
	});
...
//  Dagger specific code for initializing resource instances
	...
	// 
	if (expose) {
		model.resource = modelResource.createDefaultModelResource(name, model, schemaDescription);
	}
