...
var resourceTypes = { };
var resourceQueue = exports._queue = new EventEmitter();
...
Resource.prototype.initialize = function(parent) {
	var self = this;
	...
	if (app.express && self.allowHttp) {
		httpMeta.methods.forEach(function(method) {
			if (self[method]) {
	...
	resourceQueue.emit('ready_' + this.name, this);
	resourceQueue.emit('*', this);
};

// -------------------------------------------------------------

// 
// Checks if the given request has valid credentials to continue running. This does
// not check specific permissions/roles; That should be handled later once the initial
...
Resource.prototype.authorize = function(req) {
	...
	var verb = httpMeta.verbs[req.method];
...
Resource.prototype.createSubresource = function(name, opts) {
	opts.parent = this;
	return (this.children[name] = app.resource.create(name, opts));
};
