...
exports.initialize = function(app) {
		...
		_prepareSocket: function(method, connection) {
			...
			return function(req, res) {
				...
				if (self.route.match(parsed.pathname)) {
					self[method](
						app.Request.create(self).initializeSocket(socket, req, self.route.params, parsed.query, res)
					);
				}
			}
		},
	...
	// -------------------------------------------------------------
		...
		createSubresource: function(name, opts) {
			opts.parent = this;
...
// -------------------------------------------------------------
	...
	app.Resource.HttpError = require('./http-error');
