...
// 
Resource.prototype.authorize = function(req, promise) {
	promise = promise || new oath();

	process.nextTick(this._authorize.bind(this, req, promise));

	return promise.promise || promise;
};

Resource.prototype._authorize = function(req, promise) {
	...
	req.auth.hasPermission(this.name + '.' + verb, req).then(
		function(hasPerm) {
			promise.resolve(hasPerm || 403);
		},
		function(err) {
			promise.reject(err);
		});
