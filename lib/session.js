...
Session.prototype.hasPermission = function(perm) {
	...
	perm = perm.split('.');
	if (perm.length !== 2) {
		throw new Error('Session::hasPermissions - Invalid permission given for lookup');
	}
	...
	if (! tree) {
		return this._testGlobPermission(action, args);
	}
	...
	if (typeof tree === 'object' && tree) {
		...
		if (typeof value === 'string') {
			value = this.user[value].apply(this.user, args);
		}

		return (!! value) || this._testGlobPermission(action, args);
	}
...
// users.
// 
Session.prototype._testGlobPermission = function(action, args) {
	var perm = this.perms['*'];
	if (! action || ! perm) {
		return false;
	}
