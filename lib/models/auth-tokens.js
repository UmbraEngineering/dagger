...
var User; app.models.require(conf.userModel).resolve(function(Model) {
	User = Model;
});

var AuthToken = app.models.create('auth-tokens', {
	...
	schema: {
		...
		// Native TTL requires mongodb >= 2.4.0
		expires: {type: Date, expires: conf.authTokenTTL, default: Date.now}
	},
...
//  Logout
	...
	del: function(req) {
		...
		AuthToken.remove({_id: id}, function(err, count) {
			if (err) {
				return callback(
					new app.Resource.HttpError(500, err)
				);
