				...
				function(scopeChain) {
					...
					object.save(function(err) {
						...
						last.model.findById(object._id, function(err, object) {
							if (! parent || ! app.conf.endpoints.subresources.autoAdd) {
							...
							parent.model.findById(parent.value._id, function(err, parent) {
								if (err) {
									return promise.reject(
										new app.Resource.HttpError(500, err)
									);
					...
					async.map(data,
						function(id, done) {
							model.remove({_id: id}, function(err, count) {
								...
								if (! parent || ! app.conf.endpoints.subresources.autoRemove) {
									return done();
								}

								// Remove the reference from the parent object if this is a subresource route
								parent.model.findById(parent.value._id, function(err, parent) {
									if (err) {
										return done(
											new app.Resource.HttpError(500, err)
