	...
	//  GET
		...
		_fetchScope: function(uri) {
			...
			switch (uri[2]) {
				...
				default:
					...
					async.forEach(uri.slice(2),
						function(segment, next) {
							// If previous is a model
							if (typeof previous === 'function' && previous.findById) {
								previous.findById(segment, function(err, obj) {
									if (err) {
										return promise.reject(
											new app.Resource.HttpError(500, err)
										);
							...
							// If previous is an instance of a model
							else if (previous instanceof app.models.mongoose.Model) {
								previous = previous[segment];
								scopeChain.push(previous);
							}

							// If previous is an array
							else if (previous instanceof app.models.types.Array) {
								// 
							}

							// If previous is an object ID
							else if (false) {
								// 
							}

							else {
								throw 'Unexpected URI segment';
							}
						},
						function(err) {
							if (err) {
							...
							promise.resolve(scopeChain.pop());
						});
				break;
			}
		...
		_get: function(req) {
			...
			if (id) {
				...
				if (id === 'schema') {
					if (app.conf.app.schemaEndpoints) {
						promise.resolve(schemaDescription);
					} else {
						promise.reject(
							new app.Resource.HttpError(404, 'Document not found')
